...
export default function AllOrder() {
  ...
  const fetchOrders = async (page = 1, searchText = "") => {
    ...
    try {
      ...
      let url = `${baseURL}/api/v1/get-courier-order-lists?user_id=${user_id}&page=${page}`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.get(`${baseURL}/api/v1/delete-courier-order-agent/${id}`);
  ...
  const fetchAgents = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`${baseURL}/api/v1/get-courier-agents`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const openSetAgentModal = (orderId) => {
    setSelectedOrder(orderId);
    setSelectedAgent("");
    fetchAgents();
  ...
  const handleAssignAgent = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(
        `${baseURL}/api/v1/set-order-agent`,
        { order_id: selectedOrder, agent_id: selectedAgent },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <div className="content-wrapper p-4">
                      ...
                      <tr key={order.id}>
                        ...
                        <td>
                          ...
                          <button
                            ...
                            onClick={() => openSetAgentModal(order.id)}
                          >
                            Set Agent
                          </button>
                          <button
        ...
        >
          ...
          <div className="text-end">
            <button className="btn btn-secondary me-2" onClick={() => setIsRiderModalOpen(false)}>
              Close
            </button>
        ...
        <ReactModal
          ...
          onRequestClose={() => setIsAgentModalOpen(false)}
          className="modal-container"
          overlayClassName="modal-overlay"
        >
          <h5>Assign Agent</h5>
          ...
          <div className="text-end">
            <button className="btn btn-secondary me-2" onClick={() => setIsAgentModalOpen(false)}>
              Close
            </button>
            <button className="btn btn-success" onClick={handleAssignAgent}>
              Assign Agent
            </button>
          </div>
        </ReactModal>
